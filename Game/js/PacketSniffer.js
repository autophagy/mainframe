  ...
  PacketStream.prototype.addPacketRepeat = function() {
    if (this.context.ready) {
      if (this.queuedPacket.length <= 0) {
        ...
        this.addPacket(x);
        this.context.game.time.events.add((Phaser.Timer.SECOND / 4) / this.speed, function() {
          this.addPacketRepeat();
        }, this);
      } else {
        x = this.direction == 1 ? 0 - (15 * (this.queuedPacket.length - 1) - 3) : this.context.game.width + 3;
        this.addKeyPacket(x);
        this.context.game.time.events.add(((Phaser.Timer.SECOND / 4) / this.speed) * this.queuedPacket.length, function() {
          this.addPacketRepeat();
        }, this);
        this.queuedPacket = '';
      }
